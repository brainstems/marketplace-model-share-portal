...
const Card: React.FC<CardProps> = ({ title, className, date, time }) => {
  ...
  useEffect(() => {
    ...
    if (period === 'PM' && hoursIn24 < 12) hoursIn24 += 12;
    if (period === 'AM' && hoursIn24 === 12) hoursIn24 = 0;


    const artTime = new Date(`${date} ${hoursIn24}:${minutes} GMT-0300`);
    
 
    const localTimeString = artTime.toLocaleTimeString([], { 
      hour: '2-digit', 
      minute: '2-digit', 
      hour12: true 
    });
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-[8.5rem] md:bottom-[6.5rem] w-[10rem] right-[-4rem] border border-[#797979] rounded-xl px-4 py-2 bg-[#00000049] backdrop-blur-[10px] border-r-0">
        <p className="text-[.9rem] font-semibold uppercase">{localTime || time}</p>
      </div>
      <div className="text">
        <h1>{title}</h1>
      </div>
